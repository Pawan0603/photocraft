...
import { CirclePlus, EllipsisVertical, PenLine, SquarePen, Trash2 } from 'lucide-react'
...
function Page() {
  ...
  const handleRename = async (ImageId, newName) => {
    console.log("Renaming image with ID:", ImageId, "to new name:", newName);
    setLoading(true);
    try {
      let res = await axios.put('/api/image', { token, imageId: ImageId, name: newName });
      // console.log(res.data);
      toast({
        title: "Image renamed",
        description: "Your image has been renamed successfully.",
      });
      fetchImages(token);
    } catch (error) {
      toast({
  ...
  const handleDelete = async (Img_id, public_id) => {
    ...
    setLoading(true);
    try {
      let res = await axios.delete('/api/image', { data: { token, imageId: Img_id, publicId: public_id } });
      toast({
        title: "Image deleted",
  ...
  return (
    <>
      ...
      {renameDialogShow === true && <RenameDialog handleRename={handleRename} imageName={imgData.find(img => img._id === selectedImageId)?.name} selectedImageId={selectedImageId} setRenameDialogShow={setRenameDialogShow} />}
...
function RenameDialog({ handleRename, imageName, setRenameDialogShow, selectedImageId }) {
      ...
      <form >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Rename your image to make it more descriptive.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <button type="submit" onClick={() => handleRename(selectedImageId, name)}>Save changes</button>
          </DialogFooter>
        </DialogContent>
      </form>
    </Dialog>
